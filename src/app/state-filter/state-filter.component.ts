...
export class StateFilterComponent {
  selectedState: string = 'AL'; // Variable to hold selected state value
    ...
    this.http.get<any[]>(apiUrl).subscribe(
      (data: any[]) => {
        ...
        if (filteredData.length > 0) {
          this.dailyCases = filteredData[0].positiveIncrease || 0;
          const percentChange = (((filteredData[1].positiveIncrease - filteredData[0].positiveIncrease) / filteredData[1].positiveIncrease) * 100).toFixed(2);
          this.percentChange = parseFloat(percentChange);
